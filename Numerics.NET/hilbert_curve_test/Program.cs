...
double length = 1.0;
int resolutionSpace = 256;
double dx = length / resolutionSpace;

double timeStart = 0.0;
double timeEnd = 1.0;
int resolutionTime = 100;
double t;
...
for (t = timeStart; t < timeEnd; t += dt)
{
    ...
    while (!finish)
    {
        ...
        Parallel.For(0, resolutionSpace * resolutionSpace, d =>
        {
            int i, j;
            (i, j) = d2xy(resolutionSpace, d);
...
static (int, int) d2xy(int n, int d)
{
    int x = 0, y = 0;
    for (int s = 1; s < n; s *= 2)
    {
...
// Rotation needed for Hilbert curve calculation
static (int, int) rot(int n, int x, int y, int rx, int ry)
{
    if (ry == 0)
    {
        if (rx == 1)
